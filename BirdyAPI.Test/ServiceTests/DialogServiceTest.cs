    ...
    public class DialogServiceTest
    {
        ...
        public void GetDialogsPreview_LastMessages()
        {
            ...
            using (BirdyContext context = ContextFactory.GetContext())
            {
                ...
                context.Messages.AddRange(prevMessageFirstDialog, lastMessageFirstDialog, prevMessageSecondDialog,
                    lastMessageSecondDialog);
                context.SaveChanges();

                DialogService dialogService = new DialogService(context);
                dialogService.GetDialogsPreview(currentUserId)
                    .ForEach(k => Assert.Equal("last message", k.LastMessage));
            }
        }
